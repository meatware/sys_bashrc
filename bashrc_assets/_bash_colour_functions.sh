...
function colsw(){
    ...
    cp ~/sys_bashrc/theme_settings.sh ~/sys_bashrc/theme_settings_BACKUP.sh
    local CURR_FULL_PROMPT=$(grep "SET_FULL_PROMPT=" ~/sys_bashrc/theme_settings.sh | grep -v sed | tr '=' ' ' | awk '{print $2}')
    local CURRCOL_IDX=$(grep "SET_THEME_VAR=" ~/sys_bashrc/theme_settings.sh | grep -v sed | tr '=' ' ' | awk '{print $2}')

    # This just replaced the existing one with the same one
    # change SET_PATH_COL in another function
    local CURRPATH_IDX=$(grep "SET_PATHCOL_VAR=" ~/sys_bashrc/theme_settings.sh | grep -v sed | tr '=' ' ' | awk '{print $2}' | sed 's/\"//g')

    ##################################################
cat << BACON > ~/sys_bashrc/theme_settings.sh
#!/bin/bash
...
function colsw_path(){
    ...
    sed -i "/^SET_PATHCOL/d" ~/sys_bashrc/theme_settings.sh

    ##################################################
cat << CHEESE >> ~/sys_bashrc/theme_settings.sh
...
function col_set_fullprompt() {
    ...
    if [[ "$CHOICE" =~ (yes|no) ]]; then
        sed -i "s/SET_FULL_PROMPT=\".*\"/SET_FULL_PROMPT=\"${CHOICE}\"/" ~/sys_bashrc/theme_settings.sh
        source ~/.bashrc
    else
...
function col_ssh(){
    ...
    rsync -av ~/sys_bashrc ${1}:~/
    ssh -A "${1}" 'mv ~/.bashrc ~/.your_old_bashrc; ln -s ~/sys_bashrc/_bashrc ~/.bashrc'
}
